...
def logistic_cost(x_train, y_train, w, b):
        ...
        for i in range(m):
                ...
                #temp = -y_train[i] * np.log(fx)
                #temp2 = - (1 - y_train[i])* np.log(1 - fx)
                #print(f"(1- fx) = {(1 - fx)}", f"Second part: {temp2}")
...
def gradient_descent(x_train, y_train, w, b, alpha, iters):
        ...
        for i in range(iters):
                ...
                if i < 100000:
                        J_history.append(logistic_cost(x_train, y_train, w_in, b_in))
...
y_train = np.array([1, 0, 1, 0, 0, 1, 1, 0])
